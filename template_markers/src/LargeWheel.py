...
class LargeWheel(TaskTemplate):
    def __init__(self, server, id_):
        TaskTemplate.__init__(self, server, id_, "/world", "LargeWheel")

        self._diameter = 0.4572 # this is 18in in meters
        ...
        self._pregraspMarkerName = "LargeWheelPreGrasp" + str(id_)
        self._preGraspHandle = None

        self._turnRotationInitial = self._turnRotation = 0.3
        self._turnMarkerName = "LargeWheelTurn" + str(id_)
    ...
    def initMarker(self):
        ...
        self._LargeWheelMarker = self.createMarker(self.markerCB, markerPose)
        self.attachMenuHandler(self._LargeWheelMarker)
        self.menu_handler.reApply(self.server)
        self.server.applyChanges()

    def extendMenu(self):
        self.menu_handler.setCheckState(self.menu_handler.insert("Scale LargeWheel diameter", callback=self.enableChangeLargeWheelSize), MenuHandler.UNCHECKED)
        ...
        # self._numMenuItems += 1        
        # self.menu_handler.setCheckState(self.menu_handler.insert( "Adjust hand location on LargeWheel", parent=setControl, command_type=MenuEntry.FEEDBACK, command="", callback=self.enableHandAdjustment), MenuHandler.UNCHECKED )
        # self._numMenuItems += 1


    def markerCB(self, feedback):
        wTc_old = getFrameFromPose(self._LargeWheelMarker.pose)
        ...
        self._LargeWheelMarker.pose = feedback.pose
    ...
    def setupHands(self):
        ...
        # set grasp pose to get hands in correct orientation to center of LargeWheel
        ...
        # multiply with hand poses to get full transform from turn goals to center of LargeWheel
    ...
    def calculateTransforms(self):
        ...
        cRlg2 = kdl.Rotation.RPY(3.14, -1.57-self._graspPitchOffset, 0)
        cRrg2 = kdl.Rotation.RPY(3.14, -1.57+self._graspPitchOffset, 0)
        # multiply together to get full transform from grasps to center of LargeWheel
        ...
        lmRo = kdl.Rotation.RPY(0.0, 0.0, 0.0)
        rmRo = kdl.Rotation.RPY(0.0, 0.0, 0.0)
        # multiply hand poses w/ offset with hand adjustment to get full transform from pregrasps to center of LargeWheel
        ...
        cRl = kdl.Rotation.RPY(0,0,0)
        cRr = kdl.Rotation.RPY(0,0,0)
        # multiply together to get full transform from post turn goals to center of LargeWheel
    ...
    def enableChangeLargeWheelSize(self, feedback):
    ...
    def createScaleMarker(self):
        ...
        marker.name = "ScaleLargeWheelDiameter"
        wTc = getFrameFromPose(self._LargeWheelMarker.pose)
        ...
        self.addInteractiveMarker(marker,  self.scaleLargeWheelXY)
        self.menu_handler.reApply( self.server )
        self.server.applyChanges()
    ...
    def removeScaleMarker(self):
        self.removeInteractiveMarker("ScaleLargeWheelDiameter")
        self.menu_handler.reApply( self.server )
        self.server.applyChanges()
    ...
    def scaleLargeWheelXY(self, feedback):
        scaleXFactor = 50
        diff = feedback.pose.position.x - self._lastX
        self._lastX = feedback.pose.position.x
        marker = None
        for control in self._LargeWheelMarker.controls:
        ...
        self.server.insert(self._LargeWheelMarker)
        self.calculateTransforms()        
        wTc = getFrameFromPose(self._LargeWheelMarker.pose)
        index = 0
        for hand in self._hands[LEFT]:
